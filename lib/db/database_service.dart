...
class ReminderWithPlant {
  ...
  ReminderWithPlant({required this.reminder, required this.plantCommonName, required this.plantPhotoPath});
}
...
class DatabaseService {
  ...
  Future<List<PlantRow>> getPlants() async {
    final database = await db;
    final rows = await database.query('plants', orderBy: 'createdAt DESC');
    return rows.map(PlantRow.fromMap).toList();
  }

  Future<PlantRow?> getPlant(int id) async {
    final database = await db;
    final rows = await database.query('plants', where: 'id = ?', whereArgs: [id]);
  ...
  Future<void> updateReminderSchedule({
    ...
    await database.update(
      'reminders',
      {'nextDueAt': nextDueAt, 'lastDoneAt': lastDoneAt, 'notificationId': notificationId},
      where: 'id = ?',
      whereArgs: [id],
    );
  }
  ...
  Future<void> setReminderNotificationId(int id, int? notificationId) async {
    final database = await db;
    await database.update('reminders', {'notificationId': notificationId}, where: 'id = ?', whereArgs: [id]);
  }
